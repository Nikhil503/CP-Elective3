...
def sortUSA():
    ...
    USCITIES=locations['North America']['USA']       #Return all the cities in the USA in alphabetical order
    USCITIES.sort()
    return USCITIES
...
def alphaAsia():
    ...
    ASIALOCATIONS=locations['Asia']
    print(ASIALOCATIONS)
    s=''    
    asiaCity=[]   
    for i in ASIALOCATIONS:        
        s=s+str(ASIALOCATIONS[i][0])+' - '+str(i)        
        asiaCity.append(s)        
        s=''    
        asiaCity.sort()    #Return all the cities in Asia continent in alphabetical order
    return asiaCity
